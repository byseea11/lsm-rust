...
use crate::{
    ...
    iterators::{sst_merge_iterator::SstMergeIterator, StorageIterator},
...
fn test_task1_full_compaction() {
    ...
    sync(&storage);
    assert_eq!(storage.arch.read().l0_sstables.len(), 3);
    let mut iter = construct_merge_iterator_over_storage(&storage.arch.read());
    if TS_ENABLED {
    ...
    assert!(storage.arch.read().l0_sstables.is_empty());
    let mut iter = construct_merge_iterator_over_storage(&storage.arch.read());
    if TS_ENABLED {
        check_iter_result_by_key(
            &mut iter,
...
fn test_task2_concat_iterator() {
    ...
    for key in 0..120 {
        let iter = SstMergeIterator::create_and_seek_to_key(
            sstables.clone(),
    ...
    let iter = SstMergeIterator::create_and_seek_to_first(sstables.clone()).unwrap();
    assert!(iter.is_valid());
    assert_eq!(iter.key().for_testing_key_ref(), b"00010");
}
...
fn test_task3_integration() {
    ...
    storage.force_full_compaction().unwrap();
    assert!(storage.arch.read().l0_sstables.is_empty());
    assert!(!storage.arch.read().levels[0].1.is_empty());
